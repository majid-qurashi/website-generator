...
export default async function CustomDomainSchoolPage({ params }: PageProps) {
  const { domain } = await params;
  const decodedDomain = decodeURIComponent(domain).toLowerCase();

  // Fetch school data from Supabase using custom_domain
  ...
  if (error || !school) {
    console.error('Error fetching school by custom domain:', error);
    return notFound();
  }
